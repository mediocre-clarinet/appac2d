function [foils,wakes,Cp,xc] = appac2(surfaces,alphaDeg,xDisk,CT,options)
nSurfs = numel(surfaces);
R = [cosd(alphaDeg) -sind(alphaDeg);sind(alphaDeg) cosd(alphaDeg)];

% Determine the total number of surface panels so that we can preallocate %%%%
% space for variables                                                     %%%%
M = 0;
...
[wakes,foils.gamma] = solveWake(foils,inv(A),RHS,CT,options);

[U,V] = influence(foils.co,wakes,pi);
D = U.*cos(foils.theta) + V.*sin(foils.theta);

Qtan = B*foils.gamma + D*wakes.gamma + cos(foils.theta);
Cp = 1 - Qtan.^2;
xc = foils.co*R(1,:).';
